...
from app.chat.message import ChatResponse
...
def print_header():
    ...
    console.print()
    console.print("  [bold cyan]A powerful, local CLI for your authenticated ChatGPT session.[/bold cyan]")
    console.print("  [dim]Use it to manage prompts, search history, and script AI tasks directly from your terminal.[/dim]")
    console.print()
    console.print("  [status.done]●[/status.done] [bold white]Connected to ChatGPT[/bold white] [dim]— Edge CDP :9222[/dim]")
    console.print()
    console.print("  [bold]How to use:[/bold]")
    console.print("  [dim]•[/dim] [bold magenta]Type manually[/bold magenta] and press Enter to chat directly.")
    console.print("  [dim]•[/dim] Type [bold cyan]/prompts[/bold cyan] to access your template library.")
    console.print("  [dim]•[/dim] Type [bold cyan]/help[/bold cyan] to view all available commands.")
...
def print_response_stats(response: ChatResponse):
    ...
    console.print(f"  [meta.label]TTFT[/meta.label]       [meta.value]{response.timing.ttft_ms:,.0f} ms[/meta.value]")
    console.print(f"  [meta.label]Total[/meta.label]      [meta.value]{response.timing.total_ms / 1000:,.2f} s[/meta.value]")
    console.print(f"  [meta.label]Words[/meta.label]      [meta]{response.word_count:,}[/meta]")
    console.print(f"  [meta.label]Characters[/meta.label] [meta]{response.char_count:,}[/meta]")
    console.print()
...
class ImageGenerationLivePanel:
    ...
    def __rich__(self):
        ...
        content = f"{spinner} {self.current_status}\n\n[dim]Elapsed: {elapsed:.1f} s[/dim]"
        return Panel(content, title="IMAGE GENERATION", border_style="magenta", padding=(1, 2))
