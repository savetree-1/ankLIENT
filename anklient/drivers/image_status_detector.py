...
class ImageGenerationStatusDetector:
    ...
    def run(
        self,
        on_event: Callable[[ImageGenerationStatusEvent], None],
        timeout_sec: int = 180,
    ):
        """
        ...
        while time.time() < deadline:
            ...
            if element.count() > 0 and element.first.is_visible():
                ...
                if status and status != previous_status:
                    on_event(
                        ImageGenerationStatusEvent(
                            status=status, status_removed=False, timestamp=time.time()
                        )
                    )
                ...
                if missing_count > 15:
                    on_event(
                        ImageGenerationStatusEvent(
                            status=None, status_removed=True, timestamp=time.time()
                        )
                    )
                    return
