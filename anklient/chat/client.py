...
class ChatGPTClient:
    ...
    def send_message(
        ...
        before_last_text = (
            self.page.last_assistant_text().strip() if before_count > 0 else ""
        )
        ...
        from anklient.drivers.image_status_detector import (
            ImageGenerationStatusDetector,
            ImageGenerationStatusEvent,
        )
        ...
        if image_generating:
            from anklient.ui.terminal import ImageGenerationLivePanel
            ...
            while time.time() < deadline:
                ...
                if current_count > before_count:
                    ...
                    current = (
                        self.page.last_assistant_text().strip()
                        if current_count > 0
                        else ""
                    )
            ...
            for img in newly_added:
                ...
                if src:
                    try:
                        ...
                        if resp.status == 200:
                            download_dir = (
                                pathlib.Path.home() / "Downloads" / "ankLIENT_Images"
                            )
