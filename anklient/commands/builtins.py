...
def cmd_help(context, *args):
    ...
    console.print("   [bold cyan]/generate[/bold cyan]   Generate an image using DALL-E (boots the visual engine)")
    console.print("   [bold cyan]/memories[/bold cyan]   List all personal memories stored in your ChatGPT account")
    console.print("   [bold cyan]/projects[/bold cyan]   List your ChatGPT projects and workspaces")
...
def cmd_status(context, *args):
    ...
    if hasattr(client, 'page'):
        ...
        # API Mode
        console.print("│ Mode          REST API Client                                │")
...
def cmd_recover(context, *args):
    ...
    if hasattr(client, 'page'):
        ...
        console.print("[dim]The daemon auto-recovers. If it crashed, restart the ankLIENT Daemon.[/dim]")
...
def cmd_file(context, *args):
    ...
    if hasattr(client, 'attach_file'):
        ...
        console.print("[error]File uploads are currently unsupported in API mode.[/error]")
...
def cmd_download(context, *args):
    ...
    try:
        ...
        if save_path:
            ...
            console.print(f"[status.done]✓ Saved ({size:,} bytes) → {save_path}[/status.done]")
        else:
...
def cmd_memories(context, *args):
    ...
    if not hasattr(client, 'get_memories'):
        console.print("[error]Memories are only supported when using the APIClient.[/error]")
        return
    ...
    try:
        ...
        for m in memories:
            content = m.get("content", {}).get("memory_text", "Unknown") if isinstance(m.get("content"), dict) else str(m.get("content", ""))
...
def cmd_projects(context, *args):
    ...
    if not hasattr(client, 'get_projects'):
        console.print("[error]Projects are only supported when using the APIClient.[/error]")
        return
...
def cmd_generate(context, *args):
    if not args:
        console.print("[error]Missing prompt. Usage: /generate <image description>[/error]")
        return
    ...
    try:
        with BrowserConnection(settings.cdp_url) as conn:
            ...
            response = visual_client.send_message(prompt, on_status=print_status, on_stream=handle_stream)
            stream_panel.stop()
        ...
        console.print("[dim]Make sure Microsoft Edge is running with remote debugging on port 9222.[/dim]")
