...
from anklient.commands.builtins import register_builtins
from anklient.commands.router import CommandRouter
from anklient.files.reader import read_stdin
from anklient.history.database import init_db
from anklient.history.models import HistoryItem
from anklient.history.repository import HistoryRepository
from anklient.prompts.manager import PromptManager
from anklient.ui.terminal import (
    console,
    print_error,
    print_header,
    print_request,
...
def run_interactive(client, history_repo, prompt_mgr):
    ...
    while True:
        ...
        try:
            ...
            from anklient.ui.terminal import TextStreamingLivePanel
...
def main():
    ...
    parser.add_argument(
        "--history", action="store_true", help="Show recent history and exit"
    )
    parser.add_argument(
        "--prompts", action="store_true", help="List saved prompts and exit"
    )
    ...
    try:
        ...
        if args.history:
            from anklient.commands.builtins import cmd_history

            cmd_history({"history_repo": history_repo})
            return

        if args.prompts:
            from anklient.commands.builtins import cmd_prompts

            cmd_prompts({"prompt_mgr": prompt_mgr})
            return

        console.print("[dim]Connecting to ankLIENT Daemon...[/dim]")
        from anklient.chat.api_client import APIClient
        ...
        if args.prompt:
            ...
            if not prompts:
                console.print(
                    f"[error]Prompt template '{args.prompt}' not found.[/error]"
                )
                return
            template = prompts[0]
            ...
            if "{{" in template.template:
                if not query_str:
                    console.print(
                        f"[error]Prompt template '{template.name}' requires input.[/error]"
                    )
                    return
                from anklient.prompts.templates import (
                    extract_variables,
                    render_template,
                )
        ...
        if query_str:
            ...
            response = client.send_message(
                query_str, on_status=print_status, on_stream=handle_stream
            )
            stream_panel.stop()
            print_response(response)
